...
void amVK_Instance::ExportJSON(void) {
    ...
    for (int i = 0, lim = amVK_GPU_List.n; i < lim; i++) {
        ...
        for (int j = 0, lim = amVK_2D_GPUs_QFAMs[i].n; j < lim; j++) {
            ...
            QFam.Set("queueFlags", amVK_2D_GPUs_QFAMs[i][j].queueFlags);
            QFam.Set("queueCount", amVK_2D_GPUs_QFAMs[i][j].queueCount);
            QFam.Set("timestampValidBits", amVK_2D_GPUs_QFAMs[i][j].timestampValidBits);

            UJObject minImageTransferGranularity = UJObject::NewObject();
            minImageTransferGranularity.Set("width", amVK_2D_GPUs_QFAMs[i][j].minImageTransferGranularity.width);
            minImageTransferGranularity.Set("height", amVK_2D_GPUs_QFAMs[i][j].minImageTransferGranularity.height);
            minImageTransferGranularity.Set("depth", amVK_2D_GPUs_QFAMs[i][j].minImageTransferGranularity.depth);
